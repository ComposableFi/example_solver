use crate::{
    chains::{get_token_info, OperationInput, OperationOutput},
    json,
    routers::paraswap::{simulate_swap_paraswap, ParaswapParams},
...
pub async fn handle_ethereum_execution(
    ...
    let rpc_url = &ctx.envvars.ethereum_rpc;
    let private_key = &ctx.envvars.ethereum_key;
    let target_address: Address =
        Address::from_str(ctx.envvars.solver_addresses.get(0).unwrap()).unwrap();
    ...
    {
        if let Err(e) =
            ethereum_trasnfer_swap(ctx.clone(), &intent_id.to_string(), intent.clone(), amount)
                .await
        {
            return Err(format!(
    ...
    if !token_out.eq_ignore_ascii_case(usdt_contract_address) {
        if let Err(e) =
            approve_erc20(rpc_url, private_key, &token_out, ESCROW_SC_ETHEREUM, amount).await
        {
            tracing::error!("Error approving {token_out} for solver: {e}");
    ...
    .await
    {
        tracing::error!("Error occurred on Ethereum send token_out -> user & user sends token_in -> solver (solver must approve USDT to Escrow SC first): {}", e);
        return Err(e.to_string());
    // swap token_in -> USDT
    } else if do_swap
        && intent.src_chain == intent.dst_chain
    ...
    {
        if let Err(e) = approve_erc20(rpc_url, private_key, &token_in, PARASWAP, &amount_in).await {
            tracing::error!("Error approving {token_in} for solver: {e}");
            return Err(e.to_string());
        }
        ...
        let (token_out, token1_decimals) = match get_token_info("USDT", "ethereum") {
            ...
            None => {
                tracing::error!("Failed to get token info for USDT on Ethereum");
        ...
        let paraswap_params = ParaswapParams {
            ...
            wallet_address: Address::from_str(ctx.envvars.solver_addresses.get(0).unwrap())
                .unwrap(),
            receiver_address: Address::from_str(ctx.envvars.solver_addresses.get(0).unwrap())
                .unwrap(),
        ...
        if let Err(e) = send_tx(ctx.clone(), res_to, &res_data, 1, 500_000, 0, rpc_url).await {
            tracing::error!("Error sending transaction on Ethereum: {}", e);
            return Err(e.to_string());
        }
    }
...
pub async fn ethereum_trasnfer_swap(
    ...
    match intent.function_name.as_str() {
        ...
        "swap" => {
            ...
            let provider = Provider::<Http>::try_from(rpc_url.replace("wss", "https"))
            ...
            let tx_hash = send_tx(ctx.clone(), res_to, &res_data, 1, 500_000, 0, rpc_url).await;
...
pub async fn send_tx(
    ...
    let private_key = match secp256k1::SecretKey::from_str(&ctx.envvars.ethereum_key) {
        Ok(val) => val,
        Err(err) => return Err(format!("Failed to parse private key: {}", err)),
    };

    // Get gas
    let response = reqwest::get("https://api.etherscan.io/api?module=gastracker&action=gasoracle")
        .await
...
pub async fn get_evm_token_decimals(erc20: &ERC20<Provider<Http>>) -> u8 {
    match erc20.decimals().call().await {
        ...
        Err(e) => {
            tracing::error!("Error getting decimals: {}", e);
            0
        }
    }
}
...
pub async fn ethereum_simulate_swap(
    ...
    let rpc_url = &ctx.envvars.ethereum_key;
    ...
    let token1_decimals = get_evm_token_decimals(&ERC20::new(token_out, provider.clone())).await;
    ...
    let paraswap_params = ParaswapParams {
        ...
        token0_decimals: token0_decimals as u32,
        token1_decimals: token1_decimals as u32,
        wallet_address: Address::from_str(ctx.envvars.solver_addresses.get(0).unwrap()).unwrap(),
        receiver_address: Address::from_str(ctx.envvars.solver_addresses.get(0).unwrap()).unwrap(),
        client_aggregator: Client::new(),
    };

    let (res_amount, _, _) = simulate_swap_paraswap(paraswap_params).await.unwrap();
